import { registerSuiSnapWallet } from "@kunalabs-io/sui-snap-wallet";
import { useConnectWallet, useWallets } from "@mysten/dapp-kit";
import type { Wallet } from "@wallet-standard/core";
import { useEffect, useRef, useState } from "react";
import RecommendInstallWalletItem from "./RecommendInstallWalletItem";

interface WalletsSelectionProps {
  onSelect: (wallet: Wallet) => void;
}

const RecomennedInstallWallets = [
  {
    name: "Slush",
    value: "Slush",
    url: "https://chromewebstore.google.com/detail/slush-%E2%80%94-a-sui-wallet/opcgpfmipidbgpenhmajoajpbobppdil",
    icon: "data:image/svg+xml;base64,",
  },
  {
    name: "OKX Wallet",
    url: "https://chromewebstore.google.com/detail/okx-wallet/mcohilncbfahbmgdjkbpemcciiolgcge",
    icon: "data:image/png;base64,",
  },
  {
    name: "Binance",
    url: "https://www.binance.com/en/binancewallet",
    icon: "data:image/svg+xml;charset=utf-8;base64,",
  },
  {
    name: "Gate Wallet",
    url: "https://chromewebstore.google.com/detail/gate-wallet/cpmkedoipcpimgecpmgpldfpohjplkpp?hl=en",
    icon: "data:image/png;base64,",
  },
  {
    name: "Newmoney",
    url: "https://chromewebstore.google.com/detail/newmoneyai/coknkdplmddfgoggddpienehfhdgegna",
    icon: "data:image/svg+xml;base64,",
  },
  {
    name: "SafePal",
    url: "https://chrome.google.com/webstore/detail/safepal-extension-wallet/lgmpcpglpngdoalbgeoldeajfclnhafa",
    icon: "data:image/svg+xml;base64,",
  },
  {
    name: "Bitget Wallet",
    url: "https://chromewebstore.google.com/detail/bitget-wallet-formerly-bi/jiidiaalihmmhddjgbnbgdfflelocpak",
    icon: "data:image/png;base64,",
  },
];

export default function WalletsSelection({ onSelect }: WalletsSelectionProps) {
  const wallets = useWallets();

  const { mutate: connect } = useConnectWallet();
  const [mounted, setMounted] = useState(false);

  const isMetamaskRegistered = useRef(
    wallets?.findIndex((w: any) => w.name === "Sui MetaMask Snap") !== -1
  );

  useEffect(() => {
    setMounted(true);
  }, []);

  useEffect(() => {
    if (
      mounted &&
      typeof window !== "undefined" &&
      !isMetamaskRegistered.current &&
      (window as any).ethereum
    ) {
      registerSuiSnapWallet();
      isMetamaskRegistered.current = true;
    }
  }, [mounted]);

  if (!mounted) {
    return null;
  }

  return (
    <>
      <div className="flex w-full items-center justify-start p-3">
        <p className="text-text-sm font-medium text-gray-300">Select Wallet</p>
      </div>
      <div className="h-px w-full bg-[#17181D]" />
      {wallets
        ?.sort((a, b) => {
          if (a.name.includes("Slush")) {
            return -1;
          }
          return 1;
        })
        .map((wallet) => (
          <button
            key={wallet.name}
            type="button"
            className="flex w-full cursor-pointer items-center justify-between bg-transparent p-3 transition-all duration-300 hover:bg-gray-800"
            onClick={() => {
              connect({ wallet });
              onSelect(wallet);
            }}
          >
            <div className="flex items-center gap-3">
              <img
                src={wallet.icon}
                alt={wallet.name}
                width={24}
                height={24}
                className="rounded-full"
              />
              <p className="text-text-sm font-medium text-gray-300">
                {wallet.name}
              </p>
            </div>
          </button>
        ))}
      {RecomennedInstallWallets.filter(
        (rec) => !wallets?.find((w) => w.name === rec.name)
      ).map((rec) => (
        <RecommendInstallWalletItem key={rec.name} {...rec} />
      ))}
    </>
  );
}
